...
while(True):
    ...
    # ret, img_binary = cv2.threshold(img_gray, low, 255, cv2.THRESH_BINARY_INV) # 뒤집기 BINARY_INV
    ret, img_binary = cv2.threshold(img_gray, low, 255, cv2.THRESH_BINARY)
    cv2.imshow("Binary", img_binary)

    img_result = cv2.bitwise_and(img_color, img_color, mask = img_binary) # mask를 이용해서 원본 이미지와 바이너리 이미지 합치기
    # and 연산 시 일반적으로 물체를 흰색으로 남긴다, 이를 위해서 binary_inv
